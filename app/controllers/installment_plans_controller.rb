class InstallmentPlansController < ApplicationController
  ...
  def destroy
    @installment_plan.update!(status: :closed)
    redirect_to installment_plans_path, notice: "Plano de parcelamento desativado com sucesso."
  end

  def toggle_active
    new_status = @installment_plan.active? ? :paused : :active
    @installment_plan.update!(status: new_status)
    status_label = @installment_plan.active? ? "ativado" : "desativado"
    redirect_to installment_plans_path, notice: "Plano #{status_label} com sucesso."
  end
  ...
  def installment_plan_params
    permitted = params.require(:installment_plan).permit(
      ...
      :starts_on, :recurrence_frequency,
      :notes, :status
    )

    [ :from_account_id, :to_account_id ].each do |field|
      raw_id = params.dig(:installment_plan, field)
    ...
    if (raw_cat_id = params.dig(:installment_plan, :category_id)).present?
  ...
  def generate_first_installment
    CreateTransactionService.call(
      ...
      event_date: @installment_plan.starts_on,
      payment_date: @installment_plan.starts_on,
      category_id: @installment_plan.category_id,
      entries_attributes: [
        { account_id: @installment_plan.from_account_id, entry_type: "debit", amount: @installment_plan.installment_amount },
        { account_id: @installment_plan.to_account_id, entry_type: "credit", amount: @installment_plan.installment_amount }
      ],
    ...
    Rails.logger.warn("generate_first_installment failed: #{e.message}")
  end
end
