...
The projection services (`RecurringProjectionService`, `InstallmentProjectionService`, `MonthlyBalanceProjectionService`, `CategoryProjectionCalculator`) compute non-persisted forecasts of future transactions and balances. The design decisions behind them, including exact-date horizon semantics, the 3-month versus 6-month defaults, competence versus cash, why nothing is persisted, and the known limitations, are documented in [Projection Services](PROJECTION_SERVICES.md).
