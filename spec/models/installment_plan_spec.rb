...
RSpec.describe InstallmentPlan, type: :model do
  ...
  describe 'validations' do
    ...
    it { is_expected.to validate_presence_of(:installment_count) }
    it { is_expected.to validate_presence_of(:recurrence_frequency) }
    it { is_expected.to validate_presence_of(:starts_on) }
    it { is_expected.to validate_presence_of(:status) }

    it { is_expected.to validate_numericality_of(:installment_count)
           .is_greater_than(0)
  ...
  describe 'scopes' do
    ...
    describe '.active_plans' do
      it 'returns only active plans' do
        expect(described_class.active_plans).to include(active_plan)
        expect(described_class.active_plans).not_to include(paused_plan, closed_plan)
      end
    end

    describe '.by_frequency' do
      it 'filters by frequency' do
        expect(described_class.by_frequency('weekly')).to include(weekly_plan)
        expect(described_class.by_frequency('monthly')).to include(monthly_plan)
        expect(described_class.by_frequency('monthly')).not_to include(weekly_plan)
      end
    end

    describe '.monthly_plans' do
      it 'returns only monthly plans' do
        monthly_plans = described_class.monthly_plans
    ...
    describe '.with_pending_installments' do
      it 'returns plans with pending installments' do
        ...
        result = described_class.with_pending_installments
