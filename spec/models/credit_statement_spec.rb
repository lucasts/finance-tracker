...
RSpec.describe CreditStatement, type: :model do
  ...
  describe 'validations' do
    ...
    it { is_expected.to validate_presence_of(:amount_paid) }
  ...
  describe 'associations' do
    ...
    it 'nullifies transactions when deleted' do
      ...
      expect { credit_statement.destroy }.to change(described_class, :count).by(-1)
      expect(transaction.reload.credit_statement).to be_nil
    end
  end

  describe 'enums' do
    it { is_expected.to define_enum_for(:status).with_values(open: 0, paid: 1, overdue: 2) }
